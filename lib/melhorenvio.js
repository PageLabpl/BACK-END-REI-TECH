...
// pacote mínimo, se o lojista ainda não preencheu). O valor segurado é
// limitado a maxInsurance pra não pesar demais no frete de produtos caros —
// se o preço do item for menor que o teto, usa o preço normal mesmo.
function buildProductsPayload(items, defaultPackage, maxInsurance) {
  return items.map((i) => {
    const price = Number(i.price) || 0;
    const insurance = maxInsurance != null ? Math.min(price, maxInsurance) : price;
    return {
      id: i.productId,
...
async function calculateShipping({ token, fromCep, toCep, items, sandbox, defaultPackage, userAgent, maxInsurance }) {
  ...
  const body = {
    ...
    products: buildProductsPayload(items, defaultPackage, maxInsurance),
