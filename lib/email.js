...
function buildOrderConfirmationHtml(order, storeName) {
  ...
  // order.customer.addressText já vem pronto do server.js (endereço
  // estruturado formatado como uma linha legível).
  const enderecoTexto = order.customer.addressText || "";
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;margin:0 auto;color:#222;">
    ...
    <div style="background:#f6f7f9;border-radius:8px;padding:14px 16px;margin-top:18px;">
      ...
      <p style="margin:0;white-space:pre-line;">${escapeHtml(enderecoTexto)}</p>
    </div>
 ...
 * @param {Object} opts
 * @param {Object} opts.order - o pedido já com status "confirmado" (precisa de order.customer.addressText)
